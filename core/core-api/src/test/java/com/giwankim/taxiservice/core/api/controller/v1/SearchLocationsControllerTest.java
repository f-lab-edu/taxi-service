...
class SearchLocationsControllerTest {
  ...
  @MockBean private SearchLocationsUseCase searchLocationsUseCase;
  ...
  void searchLocations() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("keyword", "역").param("page", "1").param("size", "10"))
        .andDo(print())
        .andExpect(status().isOk());

    then(searchLocationsUseCase).should().searchLocations(new SearchLocationsQuery("역", 1, 10));
  }

  @Test
  @DisplayName("page와 size를 입력하지 않을 경우 기본값으로 조회한다.")
  void defaultValues() throws Exception {
    mockMvc.perform(get(ENDPOINT).param("keyword", "역")).andDo(print()).andExpect(status().isOk());
  ...
  void validateKeyword(String keyword) throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("keyword", keyword).param("page", "1").param("size", "10"))
        .andDo(print())
